...
public class MonthlyCronTests {
    ...
    @Test
    @DisplayName("매 월 17일 조회(주말 제외)")
    public void every17thDayOfMonth() {
        CronParser parser = CronUtils.getCronParser();
        ...
        // 초 분 시 일 월 요일 년도
        String cronString = "0 0 0 17 * ? *";
        Cron cron = parser.parse(cronString);

        List<ZonedDateTime> holidays = CronUtils.getHolidays();

        // 주말 제외 공휴일 제외 로직.
        List<ZonedDateTime> list = CronUtils.getDaysOfWholeYear();
        List<String> filtered = list.stream()
            .filter(localDateTime -> ExecutionTime.forCron(cron).isMatch(localDateTime))
            .filter(localDateTime -> !localDateTime.getDayOfWeek().equals(DayOfWeek.SATURDAY)
                && !localDateTime.getDayOfWeek().equals(DayOfWeek.SUNDAY))
            .filter(localDateTime -> !holidays.contains(localDateTime))
            .map(localDateTime -> localDateTime + "[17Day] " + localDateTime.getDayOfWeek())
            .toList();
